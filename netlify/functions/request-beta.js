...
function buildEmailPackage(submission, routing, env = process.env) {
  ...
  const factorLines = routing.scoringFactors.map(
    (item) => `- ${item.factor}: ${item.score}`
  );

  const lines = [
    "NÄLƏJ DILIGENCE",
    "Internal Intake Memo",
    "",
    "MEMO HEADER",
    `Submitted: ${submission.submittedAt}`,
    `Source: ${submission.source}`,
    `Priority: ${routing.priority}`,
    `Route: ${routing.routeTo}`,
    "",
    "CONTACT",
    ...
    `Opportunity volume: ${VOLUME_LABELS[submission.volume]} / month`,
    "Screening context:",
    submission.context,
    "",
    "QUALIFICATION DECISION",
    `Status: ${routing.status}`,
    `Recommended path: ${routing.path}`,
    `Cohort signal: ${routing.cohort}`,
    `Beta gate: ${routing.betaGate}`,
    `Next step: ${routing.nextStep}`,
    `Qualification score: ${routing.score}/100`,
    "",
    "DECISION NOTES",
    routing.notes,
    "",
